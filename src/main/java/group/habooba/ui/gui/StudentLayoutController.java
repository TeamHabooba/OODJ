...
public class StudentLayoutController {
    ...
    private void loadData() {
        ...
        for (Enrollment enrollment : core.getAllEnrollments().values()) {
            if (enrollment.studentUid() != currentStudent.uid()) {
                continue; // Skip enrollments that don't belong to this student
            }

            try {
                // Get course by UID from enrollment
                long courseUid = enrollment.course().uid();
                Course course = core.getAllCourses().get(courseUid);
